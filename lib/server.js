...
function DecodedIncomingRequest(stream) {
  this._decoder = DecodingStream({highWaterMark: 32 * 1024 * 1024});
  ...
  // Disallow writes to the stream after it is cancelled; emit a cancel event to
  // allow additional cleanup.
...
DecodedIncomingRequest.prototype._checkTimeout =
  function _checkTimeout(headers, isAllowed) {
    ...
    if (!isAllowed) {
      this._log.error('Invalid header: grpc-timeout detected in the trailers!');
...
function createServerTLS(options, requestListener) {
  ...
  options.plain = false;
  options.SNICallback = function SNICallback(servername, cb) {
    console.log('Securely connecting as: ' + servername);
...
/**
 * Patches IncomingRequest._validateHeaders
 *
 * Currently, the http2 defines 'te' to be a deprecated header.  In fact, 'te'
 * is not deprecated and rpc protocol implementations uses it to detect proxies
 * that are using older versions of the http2 protocol.
 *
 * This patch updates http2.OutgoingMessage to use an updated deprecated header
 * list
 *
 * TODO: Remove this patch, once the list of deprecated headers in corrected in
 * the http2 package
 */
IncomingRequest.prototype._validateHeaders = function _validateHeaders(headers) {
  ...
  //   PROTOCOL_ERROR.
  for (var i = 0; i < nurpc.deprecatedHeaders.length; i++) {
    var key = nurpc.deprecatedHeaders[i];
    if (key in headers) {
