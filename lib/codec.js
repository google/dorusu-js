...
/**
 * nurpc/codec provides behaviour used to encode and decode messages
...
exports.encodeMessage = function encodeMessage(message, opts, done) {
  ...
  opts = opts || {};
  var log = (opts.log || nurpc.noopLogger).child({ component: 'codec' });
  if (opts.marshal) {
    try {
...
function EncodingStream(opts) {
  ...
  this._log = (opts.log || nurpc.noopLogger).child({ component: 'codec' });
...
EncodingStream.prototype._transform = function(msg, unused_encoding, next) {
  var handleEncoded = function(encoded) {
    ...
    this._log.info({key: 'encoding'}, 'transform was called');
    this.push(encoded);
    next();
  }
...
exports.removeBinValues = function removeBinValues(key, value) {
  if (value instanceof Array) {
    var needsb64 = _.reduce(value, function(acc, v) {
      return acc || v instanceof Buffer || !isAscii.test(v);
    }, false);
    if (needsb64) {
