...
DecodedResponse.prototype._onHeaders = function _onHeaders(headers) {
  ...
  this._checkStatusHeaders(headers);
  ...
  this.stream.on('headers', function(headers) {
    that._checkStatusHeaders(headers);
    that._updateMetadata(headers);
  });
};
  ...
  _.forEach(headers, function(value, name) {
    if (!nurpc.isReservedHeader(name)) {
      if (endsWithBin.test(name)) {
        ...
        if (_.isArray(value)) {
          that.metadata[realName] = _.map(value, function(v) {
            return new Buffer(v, 'base64');
          });
        } else {
          that.metadata[realName] = new Buffer(value, 'base64');
        }
      } else {
        that.metadata[name] = value;
      }
    }
  });
