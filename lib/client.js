...
Stub.prototype.rpcFunc = function rpcFunc(opt_marshal, opt_unmarshal) {
  ...
  var doRpc = function doRpc(path, msgSrc, callback, opts) {
    ...
    _.merge(requestOpts, this.options, opts);
...
function Agent(options) {
  ...
  this._log = (options.log || nurpc.noopLogger).child({ component: 'http' });
  this.endpoints = {};
...
EncodedOutgoingRequest.prototype._start = function _start(stream, options) {
  ...
  if (typeof options.updateHeaders === 'function') {
    ...
    var withUpdatedHdrs = function(err, updatedHeaders) {
  ...
  // headers are present
  this.setHeader('user-agent', 'nurpc-nodejs/0.1')
  ...
  if (headers['grpc-timeout']) {
    ...
    this._log.info({'timeout': timeoutMicros}, 'Setting request timeout');
...
/**
 * Patches OutgoingMessage#setHeader.
 *
...
OutgoingMessage.prototype.setHeader = function setHeader(name, value) {
  if (this.headersSent) {
    ...
    if (nurpc.deprecatedHeaders.indexOf(name) !== -1) {
