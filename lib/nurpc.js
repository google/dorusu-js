...
exports.isReservedHeader = function isReservedHeader(h) {
  var reserved = exports.reservedHeaders;
  return h[0] === ':' || reserved.indexOf(h) > -1;
}

/**
 * rpcCodes are the cannonical codes shared by different implementations of
 * the rpc protocol.
 *
...
/**
 * rpcCode gives the error code given its name.
 */
exports.rpcCode = function rpcCode(name) {
  var codes = exports.rpcCodes;
  var res = codes.indexOf(name);
  if (res === -1) {
    console.log('Invalid error code name: ', name);
    throw new Error('Invalid error code name');
  }
  return res;
}

/**
 * h2Codes is a copy of the rpcCodes array found in
...
/**
 * h2NameToRpcName converts a http2 status code name to an rpc protocol
code name.
 *
 * This is used when synthesing a grpc status code and message when non-OK http2
...
// notFound is an rpc handler that terminates an rpc with status code NOT_FOUND.
...
// makeDispatcher combines rpc handlers mapped to by route names into a single
// rpc handler.
function makeDispatcher(handlers, opt_fallback) {
  handlers = handlers || {};
